...
f = open('sowpods.txt',
         'r').read().split()  # from http://stackoverflow.com/questions/13656519/python-how-to-get-rid-of-r-in-string
for i in f:
...
# For every word in valid_words, if a letter in valid_word matches a letter in temp_rack, it is taken out of temp_rack.
for word in valid_words:
    ...
    # if the number of letters matched (ie., taken out of rack_temp) equals the total number of letters in the valid word, then add the word to winning_words.
    if 7 - len(rack_temp) == len(word):
...
# add up the points (using scores dictionary) for each letter of each word in winning_words
...
print "For the maximum number of points, spell this word: " + max(winning_words_and_points, key=winning_words_and_points.get)
